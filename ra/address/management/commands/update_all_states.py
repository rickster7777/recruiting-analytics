...
class Command(BaseCommand):
    """
        ...
        /home/rishavk/Desktop/Recruiting_Analytics/ra/ra/address/management/commands/statelist.xlsx
    """
    ...
    def handle(self, *args, **options):
        ...
        try:
            for states_cities in data:
                state_name = states_cities[0].strip()
                state_abbrv = states_cities[1].strip()
                try:
                    state_obj= State.objects.get(
                        name__iexact=state_name.strip().title(),
                        abbreviation__iexact=state_abbrv.upper().strip(),
                    )
