...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # prior_lis = []
        # class_obj = Classification.objects.filter(year='2020') 
        # if len(class_obj) > 0:   
        #     player_playmaking = Player.objects.filter(classification=class_obj[0]).order_by('priority')
        div_1 = int(Player.objects.all().exclude(role__name__iexact='nfl').count()/4)    
        player_playmaking = Player.objects.filter(priority__range=[1,div_1]).order_by('priority')
                                ...
                                if profile_soup.find_all('ul', {'class': 'career-navigation nav nav-pills'}):
                                    ...
                                    for carrer_stats in football_links:
                                        ...
                                        if carrer_stats.text == 'Football' or carrer_stats.text == 'FB':
                                            ...
                                            # if playmaking_soup.find_all('ul',{'class':'featured-stats'}):
                                            #     stats_list = playmaking_soup.find_all('ul',{'class':'featured-stats'})
                                            #     datapoints_check = stats_list[0].find_all('li')
                                            #     for points in datapoints_check:
                                            #         if points.find('div',{'class':'stat-name'}):
                                            #             if 'Interceptions' in points.find('div',{'class':'stat-name'}).text:
                                            #                 Intc = points.find('div',{'class':'stat-field'}).text.replace('\r','').replace('\n','').replace('\t','')
                                            #             if 'Sacks' in points.find('div',{'class':'stat-name'}).text:
                                            #                 Sacks = points.find('div',{'class':'stat-field'}).text.replace('\r','').replace('\n','').replace('\t','')
                                            #             if 'Total TDs' in points.find('div',{'class':'stat-name'}).text:
                                            #                 touchd = points.find('div',{'class':'stat-field'}).text.replace('\r','').replace('\n','').replace('\t','')
                                            #     print(Intc, Sacks, touchd)
                                            #     print("success")
                                                                ...
                                                                if category_check.find('tr', {'class': 'first'}):
                                                                    ...
                                                                    if catg_check.find('td', {'class': 'sacks stat dw'}):
                                                                        Sacks = catg_check.find(
                                                                            'td', {'class': 'sacks stat dw'}).text
                                                                    # if catg_check.find('td',{'class':'year first'}):
                                                                        # year = catg_check.find('td',{'class':'year first'}).text
                                                                        # if '19-20' in year:
                                                                        #     if category_check.find('tr',{'class':'first last'}).find('td',{'class':'sacks stat dw'}):
                                                                        #         Sacks = category_check.find('tr',{'class':'first last totals'}).find('td',{'class':'sacks stat dw'}).text
