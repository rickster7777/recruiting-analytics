...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        div_2 = 2*int(Player.objects.all().exclude(role__name__iexact='nfl').count()/4)    
        player_playmaking = Player.objects.filter(priority__range=[div_1 + 1 ,div_2]).order_by('priority') 
