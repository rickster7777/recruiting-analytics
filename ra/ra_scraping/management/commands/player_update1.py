...
class Command(BaseCommand):
    """
        Use Case: python manage.py player_update1
    """
    ...
    def handle(self, *args, **options):
        ...
        player_prior = Player.objects.filter(priority__range=[1,div_1]).order_by('priority')     
        #player_prior = Player.objects.filter(classification__year='2022').order_by('priority')
        def get_driver():
            try:
